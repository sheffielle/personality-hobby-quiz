...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function createQuestionCard(q, index, onOptionChange, prefersReducedMotion) {
    ...
    loadQuestions().then(qs => {
        ...
        startBtn.addEventListener('click', () => {
            ...
            const staticHeader = document.querySelector('.quiz-header');
            if (staticHeader)
                staticHeader.style.display = 'none';
            const staticDescription = document.querySelector('.whimzi-desc');
        ...
        submitBtn.addEventListener("click", async (e) => {
            ...
            questions.forEach((q, index) => {
                ...
                if (selected && selected !== "Prefer not to answer") {
                    ...
                    if (nodeName) {
                        const weight = parseFloat(document.querySelector(`input[name="importance-${index}"]`)?.value || "0.5");
            ...
            try {
                ...
                let resultsContainer = document.getElementById("results-overlay");
                if (!resultsContainer) {
    ...
    <ol class="hobby-list">
      ${topHobbies.map((h, i) => `<li style="animation-delay:${(i * 0.1).toFixed(1)}s">${h}</li>`).join("")}
    </ol>
  `;
            }
            catch (err) {
