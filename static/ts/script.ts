...
type Axis = 'Mind' | 'Information' | 'Decision-Making' | 'Lifestyle' | 'Adventurousness';
interface Question {
  axis: Axis;
  text: string;
  options: string[];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const quizPage      = document.getElementById('quiz-page')         as HTMLElement;
  const summaryPage   = document.getElementById('summary-page')      as HTMLElement;
  ...
  const codeTitle     = document.getElementById('code-title')        as HTMLElement;
  const summaryDiv    = document.getElementById('summary-content')   as HTMLElement;
  ...
  const letterMap: Record<Axis, [string,string,string]> = {
    Mind:           ['O','V','R'],
    Information:    ['P','O','I'],
  ...
  function showQuestion(idx: number) {
    ...
    quizForm.querySelectorAll<HTMLElement>('.question-block').forEach(c => {
    ...
    const card = quizForm.querySelector<HTMLElement>(
      `.question-block[data-index="${idx}"]`
    )!;
  ...
  function updateProgress(): void {
    const answeredCount = quizForm.querySelectorAll<HTMLInputElement>('input[type=radio]:checked').length;
  ...
  function calculateAndDisplay() {
    ...
    const counts: Record<Axis,[number,number,number]> = {
      Mind: [0,0,0],
      Information: [0,0,0],
    ...
    data.forEach(v => {
      ...
      const ax  = s.slice(0,d) as Axis;
      const sel = parseInt(s.slice(d+1),10);
      counts[ax][sel]++;
    });

    const code = (Object.keys(counts) as Axis[])
      .map(ax => {
        const arr = counts[ax];
    ...
    const traitLabels: Record<Axis,Record<string,string>> = {
    ...
    const labels = (Object.keys(counts) as Axis[])
  ...
  nextBtn.addEventListener('click', (e) => {
    ...
    e.preventDefault();
    const card = quizForm.querySelector<HTMLElement>(
      `.question-block[data-index="${currentIndex}"]`
    )!;
    const hasAnswer = Array.from(
      card.querySelectorAll<HTMLInputElement>('input[type=radio]')
    ).some(r => r.checked);
    if (!hasAnswer) return;
